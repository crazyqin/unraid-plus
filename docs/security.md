...
AIGC:
  ...
  Label: '1'
  ProduceID: '67551ccf-f534-4854-81bc-9c2a0a2ace56'
  PropagateID: '67551ccf-f534-4854-81bc-9c2a0a2ace56'
  ReservedCode1: '10103e2a-c295-4fdf-81b9-7b1cc7d5c697'
  ReservedCode2: '10103e2a-c295-4fdf-81b9-7b1cc7d5c697'
---
...
再改 authorized_keys——unraid++ 一键搞定，且整个过程是 SSH 通道内完成的。
...
unraid++ 采用 **Trust On First Use（TOFU）**：
...
这是 unraid++ 安全性最薄弱的一环，**默认监听 `:8080` 是明文 HTTP**。
必须满足以下任一条件：
...
- [ ] **链路加密**：unraid++ 前面套了 HTTPS 反代，或部署在 VPN/Tailscale 内。
...
- [ ] **非 root 运行**：容器 / systemd 以专用 unraidpp 用户跑（Dockerfile 已内置
      `USER app`，systemd 单元见 [deployment.md §4.2](deployment.md)）。
- [ ] **最小权限 SSH**：在 Unraid 的 `/etc/ssh/sshd_config` 里把 unraid++ 用的
      密钥限制为 `restrict,pty,command="..."` 模式不现实（我们就是需要全功能 shell），
      但至少做到「只为 unraid++ 生成一对专用密钥，不共享 root 通用 authorized_keys」。
...
- 邮件：security@your-org.example（占位，发布前替换）
